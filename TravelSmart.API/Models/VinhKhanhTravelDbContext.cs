...
public partial class VinhKhanhTravelDbContext : DbContext
{
    ...
    public VinhKhanhTravelDbContext(DbContextOptions<VinhKhanhTravelDbContext> options) : base(options) { }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Poi>(entity => {
            ...
            entity.Property(e => e.QrCodeKey).HasMaxLength(100).IsUnicode(false);
            entity.Property(e => e.RadiusMeter).HasDefaultValue(50);

            // 🔥 MAP CỘT PRIORITY VÀO DB
            entity.Property(e => e.Priority).HasDefaultValue(0);

            entity.HasOne(d => d.Owner).WithMany(p => p.Pois).HasForeignKey(d => d.OwnerId).HasConstraintName("FK__POIs__OwnerId__440B1D61");
        });
        ...
        modelBuilder.Entity<PoiTranslation>(entity => {
            ...
            entity.HasOne(d => d.LanguageCodeNavigation).WithMany(p => p.PoiTranslations).HasForeignKey(d => d.LanguageCode).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__PoiTransl__Langu__4E88ABD4");
            entity.HasOne(d => d.Poi).WithMany(p => p.PoiTranslations).HasForeignKey(d => d.PoiId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__PoiTransl__PoiId__4D94879B");
        });
        ...
        modelBuilder.Entity<User>(entity => {
            ...
            entity.HasOne(d => d.Role).WithMany(p => p.Users).HasForeignKey(d => d.RoleId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__Users__RoleId__3D5E1FD2");
        });
        ...
        modelBuilder.Entity<VisitLog>(entity => {
            ...
            entity.Property(e => e.VisitTime).HasDefaultValueSql("(getdate())").HasColumnType("datetime");
            entity.Property(e => e.VisitType).HasMaxLength(20).IsUnicode(false);
            entity.HasOne(d => d.Poi).WithMany(p => p.VisitLogs).HasForeignKey(d => d.PoiId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__VisitLogs__PoiId__52593CB8");
            entity.HasOne(d => d.User).WithMany(p => p.VisitLogs).HasForeignKey(d => d.UserId).HasConstraintName("FK__VisitLogs__UserI__534D60F1");
        });
